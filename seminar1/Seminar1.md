...
i kapittel 1 i **Lær deg R** som er anbefalt litteratur.
...
Til slutt så kan vi hente ut informasjon om en variabel for en
observasjon:
...
grov inndeling av ulike typer vektorer i R (se også s. 42-47 i Lær deg
R):
...
`select()` og `filter()` fra `tidyverse`.
